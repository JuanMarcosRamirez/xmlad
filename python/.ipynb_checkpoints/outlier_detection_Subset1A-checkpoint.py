...
def cluster_anomaly_support_optimal(data, yanm, attributes, cat_cols, file_save):
    ...
    if (len(attributes) >= num_features):
        ...
        plt.title('(b) ROAD ranking (anomalies)', fontsize=14)
...
for ii in range(X.shape[1]):
    anm_labels.append('waveform_'+ str(ii))
    # anm_labels.append('cardio_'+ str(ii))
Xdf = pd.DataFrame(X,columns=anm_labels)

print('-----------------------------------------------------------------')
print('CLUSTER DENSITIES')
models, cls_mat = cluster_anomaly_support_optimal(Xdf, y, anm_labels, [],'waveform')
print(models)
...
#     bl_vec2 = (cls_vec==2)
    ...
    k = k+1
    plt.savefig('data/figures/waveform_feat_anomalies_%dN.png'%(k),dpi=150, bbox_inches='tight')
